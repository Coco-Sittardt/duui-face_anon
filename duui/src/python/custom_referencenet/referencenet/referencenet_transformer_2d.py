...
class Transformer2DModelOutput(BaseOutput):
    ...
    referencenet_states: Tuple[torch.FloatTensor, ...]
...
class Transformer2DModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        for block in self.transformer_blocks:
            if self.training and self.gradient_checkpointing:
                ...
                hidden_states, referencenet_output = block(
                    ...
                    referencenet_states=None,
                )
                referencenet_states += (referencenet_output,)
        ...
        if not return_dict:
            return (output, referencenet_states)

        return Transformer2DModelOutput(
            sample=output, referencenet_states=referencenet_states
        )
